...
#pragma omp parallel
  {
    ...
    for (l = 0; l < ppt; l++) {
      ...
      for (j = -tt; j < tt + 1; j++) {
        for (k = -tt; k < tt + 1; k++) {
          if (((xx + j) >= 0) && ((xx + j) < xsize) && ((yy + k) >= 0) &&
              ((yy + k) < ysize)) {
            local_image[(yy + k) * xsize + (xx + j)] +=
                mm * cubic_kernel(
                         sqrt((float)j * (float)j + (float)k * (float)k), tt);
          }
        }
      }
    }
...
static PyObject *rendermodule(PyObject *self, PyObject *args) {
  ...
  PyArrayObject *image_obj =
      (PyArrayObject *)PyArray_SimpleNewFromData(1, dims, NPY_FLOAT32, image);
